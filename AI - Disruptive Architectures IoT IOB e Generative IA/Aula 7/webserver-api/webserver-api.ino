...
void sendJsonTempResponse(float temperatura, float umidade) {
  String response = "{";
  response += "\"ok\":true,";
  response += "\"Temperatura\":" + String(temperatura) + ",";
  response += "\"Umidade\":" + String(umidade) + ",";
  response += "}";
...
void setup(void) {
  ...
  server.on(UriBraces("/api/temp"), HTTP_GET, []() {
    TempAndHumidity data = dht.getTempAndHumidity();
    float temperatura = data.temperature;
    float umidade = data.humidity;
    sendJsonTempResponse(temperatura, umidade);
  });
